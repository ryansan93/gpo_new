var tp = {
	...
	modalAddForm: function () {
        ...
        $.get('parameter/TaxPembelian/modalAddForm',{
        },function(data){
            var _options = {
                className : 'large',
                message : data,
	...
	save: function () {
		...
		if ( err > 0 ) {
			...
			bootbox.confirm('Apakah anda yakin ingin menyimpan data Tax Pembelian ?', function (result) {
				if ( result ) {
					var params = {
					...
					$.ajax({
			            url: 'parameter/TaxPembelian/save',
			            data: {
			                'params': params
			            },
			            type: 'POST',
	...
	delete: function (elm) {
		bootbox.confirm('Apakah anda yakin ingin meng-hapus data Tax Pembelian ?', function (result) {
			if ( result ) {
				$.ajax({
		            url: 'parameter/TaxPembelian/delete',
		            data: {
		                'id_tp': $(elm).attr('data-id')
		            },
		            type: 'POST',
		            dataType: 'JSON',
		            beforeSend: function() { showLoading(); },
